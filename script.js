// document.addEventListener('DOMContentLoaded', () => {
    ...
    let squares = Array.from(document.querySelectorAll('.grid div'))  //array.from() collects all 200 divs and put them into an array we can work with, now each div will have a different index number
    ...
    // makes tetromino move down every second 
    ...
    function rotate() {
        undraw()
        currentRotation++
        if (currentRotation === current.length) { // if the current rotation gets to 4, makeit go back to 0
            currentRotation = 0
        }
        current = theTetrominoes[random][currentRotation]
        draw()
    ...
    // display upcoming tetromino in the mini-grid 
    ...
    // adding fuctionality to the start button
